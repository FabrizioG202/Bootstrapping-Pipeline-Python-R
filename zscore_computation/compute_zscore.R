...
load_sparse_data <- function(dat_file) {
    ...
    # X2 = Bin 2 start
    # X3 = Count (Score)
    chr_dat <- suppressMessages(vroom(dat_file, delim = "\t", col_names = F, trim_ws = T, escape_double = F))
...
# Compute zscore
data = compute_zscore(path, ncluster = ncluster)

# save the table
write.table(data, file = out_file, sep = "\t", row.names = F, col.names = F)
...
### R dagger.R <pvalues_table> <features_bed> <cluster_file> <out_file>
#reading the commandline arguments
argv = commandArgs(trailingOnly=TRUE)
feature_pval_file<- argv[[1]] #pvalues_table
feature_coord_file<- argv[[2]] #features_bed
BHiCect_res_file<- argv[[3]] #cluster_file
out_tsv<- argv[[4]] #out_file

# if bed file
# feature_coord_tbl<-import.bedGraph(feature_coord_file)
valr_bed = valr::read_bed(feature_coord_file, )
feature_coord_tbl = GRanges(seqnames = valr_bed$chrom, ranges = IRanges(start = valr_bed$start, end = valr_bed$end))
...
write.table(dagger_mres_tbl,out_tsv,sep="\t",quote=F,row.names=F,col.names=T)
